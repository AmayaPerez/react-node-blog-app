...
const getPrivateData = asyncErrorWrapper((req,res,next) =>{
    ...
    return res.status(200).json({
        success:true ,
        message : "You are allow to visit this page ",
        user : req.user
...
const login  = asyncErrorWrapper (async(req,res,next) => {
    ...
    if(!validateUserInput(email,password)) {
        ...
        return next(new CustomError("Please check you credentials",400))
    }
    ...
    if(!comparePassword(password,user.password)){
        return next(new CustomError("Your password is wrong",404))
    }
...
const forgotpassword  = asyncErrorWrapper( async (req,res,next) => {
    ...
    if(!user ) {
        return next(new CustomError( "The user is no registered in our database",400))
    }
